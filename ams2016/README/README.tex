...
\title{AMS 2016: UniCloud, Docker at Unidata}
\hypersetup{
...
\section{LDM, TDS, and RAMADDA on Microsoft Azure VM}
...
The following instructions describe how to configure a \href{https://azure.microsoft.com}{Microsoft Azure VM} serving data with the \href{http://www.unidata.ucar.edu/software/ldm/}{LDM}, \href{http://www.unidata.ucar.edu/software/thredds/current/tds/}{TDS}, and \href{http://sourceforge.net/projects/ramadda/}{RAMADDA}. This document assumes you have access to Azure resources though these instructions should be fairly similar on other cloud providers (e.g., Amazon). They also assume familiarity with Unix, Docker, and Unidata technology in general. You will have to be comfortable entering command at the Unix command line. We will be using Docker images defined here:\\
...
\subsubsection{\texttt{docker-machine}}
...
\subsection{Preliminary Setup on Azure}
...
\subsubsection{Create a VM on Azure.}
...
\subsubsection{Configure Unix Shell to Interact with New Azure VM.}
...
\subsubsection{\texttt{ssh} into VM with \texttt{docker-machine}}
...
\subsubsection{Install Package(s) with \texttt{apt-get}}
...
\subsubsection{Add \texttt{ubuntu} User to \texttt{docker} Group and Restart Docker}
...
\subsubsection{Restart Azure VM}
...
\subsubsection{\texttt{ssh} into VM}
...
\subsubsection{Install \texttt{docker-compose} on VM}
...
\begin{itemize}
\item \href{https://github.com/Unidata/Unidata-Dockerfiles}{\texttt{Unidata-Dockerfiles}}
\item \href{https://github.com/Unidata/TdsConfig}{\texttt{TdsConfig}}
\end{itemize}

\begin{enumerate}
\item \texttt{Unidata-Dockerfiles}
...
\subsubsection{\texttt{git clone} Repositories}
...
\subsubsection{Configuring the LDM}
\label{sec:orgheadline24}
\begin{enumerate}
\item LDM Directories on Docker Host
...
\item Upstream Data Feed from Unidata or Elsewhere
...
\subsubsection{Configuring the TDS}
\label{sec:orgheadline26}
\begin{enumerate}
\item Edit TDS catalog.xml Files
...
When you fire up RAMADDA for the very first time, you ,ust have  a \texttt{password.properties} file in the RAMADDA home directory which is \texttt{/data/repository/}. See \href{http://ramadda.org//repository/userguide/toc.html}{RAMADDA documentation} for more details on setting up RAMADDA.

Here is a \texttt{pw.properties} file to get you going. Change password below to something more secure!

\begin{verbatim}
echo ramadda.install.password=changeme! > /data/repository/pw.properties
\end{verbatim}

\subsection{Opening Ports}
...
Note the TDM is an application that works in conjuction with the TDS. It creates indexes for GRIB data in the background, and notifies the TDS via port 8443 when data have been updated or changed. See \href{https://www.unidata.ucar.edu/software/thredds/current/tds/reference/collections/TDM.html}{here} to learn more about the TDM.
...
\subsection{Starting the LDM TDS RAMADDA TDM}
...
\subsection{Check What is Running}
